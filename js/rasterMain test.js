/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 * $Id: rasterMain\040test.js,v 1.1.2.2 2013/07/18 19:52:23 vriezekolke Exp $
 *
...
function testLocalStorage() {
	var noLS = "HTML5 local storage is not supported by this browser or configuration.\nThis app will not work properly.";
...
function initLibraryPanel() {
	...
	$('#libadd').attr('title','Add a blank project to the library.');
					...
					if (!p.stub) {
						...
						if (!p.shared && becomesShared) {
							...
							if (it.notlast()) {
								...
								p.setshared(becomesShared,false); // transactionCompleted() will take care of the server
//								if (p.id == Project.cid)
//									startAutoSave();
							}
		...
		function() {
			...
			if (t>2) {
				rasterConfirm('Delete project?',
					...
					'Yes, really remove','Cancel', dokill);
			} else {
				dokill();
			}
...
//	if (jsPlumb.setRenderMode) jsPlumb.setRenderMode(jsPlumb.SVG);
...
function paintSingleFailures(s) {
	...
	// First paint all the accordions, but do not fill them yet. They get
	// filled when they are first clicked.
	snippet = '\
	...
	for (it.first(); it.notlast(); it.next()) {
		...
		var snippet = '\n\
		  <div id="sfaccordion_SV___ID_" class="sfaccordion">\n\
			<h3><a href="#">Single failures for "_TI_" (_TY__AP_) <span id="sfamark_SV___ID_"></span></a></h3>\n\
			<div>\n\
			 <div id="sfa_SV___ID_">\n\
			 </div>\n\
		';
...
function opencloseSFAccordion(cm,sid,event,ui) {
	...
	if (!cm.accpainted) {
		...
		';
		$('#sfa'+sid+'_'+cm.id).html(snippet);
		for (var i=0; i<cm.thrass.length; i++) {
			var te = ThreatAssessment.get(cm.thrass[i]);
			te.addtablerow('#sfa'+sid+'_'+cm.id,"sfa"+sid+'_'+cm.id);
		}
	}
}
